 ...
 * Inspector controls
 */
const Inspector = (props: SvgProps) => {
	const {
		attributes: {
			width,
			height,
		...
		<InspectorControls>
			...
			>
				...
				<NumberControl
					...
					onChange={(value) => {
						setAttributes({ width: parseInt(String(value), 10) });
						if (syncHeight) {
							setAttributes({ height: parseInt(String(value), 10) });
						}
					}}
					min={16}
				/>
			...
			>
				<NumberControl
					...
					value={strokeWidth}
					onChange={(value) =>
						setAttributes({ strokeWidth: parseInt(String(value), 10) })
					}
					min={0}
				/>
			</PanelBody>
